...
pygame.font.init()
FONTES_PONTOS = pygame.font.SysFont('arial', 25)
...
class Passaro:
    ...
    def __init__(self, x, y):
        ...
        self.imagem = self.IMGS[0]  # imagem inicial do passaro
    ...
    def desenhar(self, tela):
        # definindo qual imagem será usada no personagem
        ...
        if self.contagem_imagem < self.TEMPO_ANIMACAO:
            ...
            self.imagem = self.IMGS[1]
        elif self.contagem_imagem >= self.TEMPO_ANIMACAO*2 + 1:
            self.imagem = self.IMGS[0]
            self.contagem_imagem = 0

        # quando o passaro estiver caindo não vai bater asa
        if self.angulo <= -80:
            self.imagem = self.IMGS[1]
...
def desenhar_tela(tela, passaros, canos, chao, pontos):
    ...
    texto = FONTES_PONTOS.render(f"Potuação: {pontos}", 1, (255, 255, 255))
    tela.blit(texto, (TELA_LARGURA - 10 - texto.get_width(), 10))

    if ia_jogando:
        texto = FONTES_PONTOS.render(f"Geração IA: {geracao}", 1, (255, 255, 255))
...
if __name__ == '__main__':
    ...
    caminho_config = os.path.join(caminho, 'Configuracao_Inteligencia_Artificial.txt')
